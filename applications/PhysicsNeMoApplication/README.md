...
Twenty-one fully documented use cases live in the [Examples repository](https://github.com/KratosMultiphysics/Examples/tree/master/physics_nemo_application): self-contained scripts against real Kratos solves, with every figure regenerated by the code — the two newest being [Digital twin in OpenUSD](https://github.com/KratosMultiphysics/Examples/tree/master/physics_nemo_application/use_cases/digital_twin_usd) (a transient solve, its in-loop surrogate and its MC-dropout uncertainty in one scrubbable `.usda`, the figure rendered entirely from the reopened file) and [Volumetric diffusion downscaling](https://github.com/KratosMultiphysics/Examples/tree/master/physics_nemo_application/use_cases/volumetric_diffusion) (`denoiser_interface: "unet3d"` on 16³ grids of real 3D solves: the ensemble mean halves the coarse error, and the de-normalization lesson applies in the training direction too — fields must be scaled to EDM's `sigma_data` or the sampler's own noise wins). Highlights (each image links to its case):

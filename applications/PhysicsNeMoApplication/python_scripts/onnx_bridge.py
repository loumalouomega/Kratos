...
def _TryImportOnnxExport():
    try:
        ...
        raise ImportError(
            "PhysicsNeMoApplication.onnx_bridge requires physicsnemo (and torch) for ONNX "
...
def _TryImportOnnxRuntime():
    try:
        ...
        raise ImportError(
            "PhysicsNeMoApplication.onnx_bridge requires onnxruntime, which could not be "
...
def CreateOrtSession(model, device: str = "cpu", require_device: bool = False):
    ...
    if "CUDAExecutionProvider" not in session.get_providers():
        ...
        Kratos.Logger.PrintWarning("PhysicsNeMoApplication.onnx_bridge", message)

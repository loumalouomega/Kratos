...
// The SumReduction, MaxReduction, MinReduction classes are outside Kratos namespace
// as they are meant to be general purpose and used by BlockPartition and IndexPartition
// which are also outside Kratos namespace (but inside the Kratos project).

template <class TDataType>
class SumReduction
{
public:
    typedef TDataType value_type; //used by TBB to type the reduction
    typedef TDataType return_type; //expected type of the final result

    SumReduction() : mValue(ZeroVal<TDataType>()) {}
    SumReduction(TDataType Value) : mValue(Value) {} // specialized constructor for TBB

    //************************************************************************************
    //************************************************************************************
    //this function is called by each thread locally
    void LocalReduce(const TDataType& rValue)
    {
        mValue += rValue;
    }
    ...
    //************************************************************************************
    //************************************************************************************
    //this function is called by each thread to merge all of the local reductions
    void ThreadSafeReduce(const SumReduction<TDataType>& rOther)
    {
// #if defined(_OPENMP) // Should be KRATOS_PARALLEL_FRAMEWORK_OPENMP
#if defined(KRATOS_PARALLEL_FRAMEWORK_OPENMP)
        #pragma omp critical
        {
...
#elif defined(KRATOS_PARALLEL_FRAMEWORK_TBB)
        KRATOS_CRITICAL_SECTION; // Use global lock for thread-safety
        mValue += rOther.mValue;
#else
        // This is for serial execution, CXX11, or any other case
        mValue += rOther.mValue;
#endif
    }
...
template <class TDataType, class TCompareType = std::less<TDataType>>
class MaxReduction
{
public:
    typedef TDataType value_type; //used by TBB to type the reduction
    ...
    MaxReduction() : mValue(std::numeric_limits<TDataType>::lowest()) {}
    MaxReduction(TDataType Value) : mValue(Value) {} // specialized constructor for TBB
    ...
    //************************************************************************************
    //this function is called by each thread to merge all of the local reductions
    void ThreadSafeReduce(const MaxReduction<TDataType, TCompareType>& rOther)
    {
// #if defined(_OPENMP) // Should be KRATOS_PARALLEL_FRAMEWORK_OPENMP
#if defined(KRATOS_PARALLEL_FRAMEWORK_OPENMP)
        #pragma omp critical
        {
            if (TCompareType{}(mValue, rOther.mValue))
                mValue = rOther.mValue;
        }
#elif defined(KRATOS_PARALLEL_FRAMEWORK_TBB)
        //this is not threadsafe by construction.
        //to make it threadsafe, the use of a lock is required
        KRATOS_CRITICAL_SECTION;
        if (TCompareType{}(mValue, rOther.mValue))
            mValue = rOther.mValue;
#else
    ...
    /**
     * @brief Custom join method for TBB.
     * @param rOther The other reduction object to combine with.
     */
    void join(const MaxReduction<TDataType, TCompareType>& rOther) {
        if (TCompareType{}(mValue, rOther.mValue)) {
            mValue = rOther.mValue;
        }
    }
...
template <class TDataType, class TCompareType = std::greater<TDataType>>
class MinReduction
{
public:
    typedef TDataType value_type; //used by TBB to type the reduction
    ...
    MinReduction() : mValue(std::numeric_limits<TDataType>::max()) {}
    MinReduction(TDataType Value) : mValue(Value) {} // specialized constructor for TBB
    ...
    //************************************************************************************
    //this function is called by each thread to merge all of the local reductions
    void ThreadSafeReduce(const MinReduction<TDataType, TCompareType>& rOther)
    {
// #if defined(_OPENMP) // Should be KRATOS_PARALLEL_FRAMEWORK_OPENMP
#if defined(KRATOS_PARALLEL_FRAMEWORK_OPENMP)
        #pragma omp critical
     ...
     * @param rOther The other reduction object to combine with.
     */
    void join(const MinReduction<TDataType, TCompareType>& rOther) {
        if (TCompareType{}(mValue, rOther.mValue)) {
            mValue = rOther.mValue;
        }
    }
...
namespace Kratos
{
// TODO: remove this at some point and use the "external" reducers above
// kept for backwards compatibility for now
template <class TDataType>
using SumReductionUtilities = SumReduction<TDataType>;

template <class TDataType, class TCompareType = std::less<TDataType>>
using MaxReductionUtilities = MaxReduction<TDataType, TCompareType>;

template <class TDataType, class TCompareType = std::greater<TDataType>>
using MinReductionUtilities = MinReduction<TDataType, TCompareType>;

} // namespace Kratos.

// #undef KRATOS_PREPARE_CATCH_THREAD_EXCEPTION // Moved these into the #if defined(KRATOS_PARALLEL_FRAMEWORK_OPENMP) block

...
// Eigen-backed implementation of the fixed-size dense vector array_1d,
// selected by containers/array_1d.h when KRATOS_LINEAR_ALGEBRA_BACKEND=eigen
// (KRATOS_USE_EIGEN_BACKEND). It honours the same contract as the historical
// uBLAS implementation - see tests/cpp_tests/containers/test_dense_backend_contract.cpp:
// plain T[N] layout starting at the object address, the uBLAS-style member
// surface (operator()/operator[], size, resize, iterators, ...) and the uBLAS
// text format for streaming.
//
// Do not include this header directly; include containers/array_1d.h.
...
public:
    ...
    Internals::EigenArrayProxy<T> data() { return {BaseType::data(), N}; }
    Internals::EigenArrayProxy<const T> data() const { return {BaseType::data(), N}; }

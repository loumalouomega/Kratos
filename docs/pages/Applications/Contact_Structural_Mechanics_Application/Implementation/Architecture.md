...
| `automatic_differentiation/` | The shared generator module (`mortar_condition_generator.py`), the per-family generators (`generate_*_mortar_condition.py` scripts and the documented `*.ipynb` notebooks), the `*_template.cpp` files with the `BEGIN/END AD REPLACEMENT` markers, the headless notebook runner and the numerical comparison tool, the LaTeX derivations and per-folder READMEs | [Automatic differentiation](../Theory/Automatic_Differentiation.html) |
...
|---|---|---|
| `__init__` | Stores `contact_settings` (defaults from `AuxiliaryContactSettings`); `AuxiliarySetSettings` is a no-op kept for compatibility (it used to raise `buffer_size` to 3 for frictional formulations, which is no longer needed) | `auxiliary_methods_solvers.py` |

...
| `u1old`, `u2old` | offset from `X1`/`X2` to the previous position, so that $$\mathbf{x}^{i,t} = \mathbf{X}^i + \mathbf{u}^i_{old}$$ (non-objective slip) | `DerivativeDataFrictional::Initialize/UpdateMasterPair`: `GetCoordinates(…, false, 1) - GetCoordinates(…, false, step)` |
...
(`TangentSlipObjective` / `TangentSlipNonObjective`; the $$\Delta t$$ of (4.68)–(4.69) cancels between the velocity and the increment and is commented out in the generated code). Note the sign convention: the weighted gap is $$-\mathbf{n} \cdot (\mathbf{D}\mathbf{x}^1 - \mathbf{M}\mathbf{x}^2)$$ (positive when open), and the two slip measures are defined so that they approximate the same tangential relative motion with that convention: for a slave sliding by $$\delta$$ over a fixed master both give $$-D_{jj}\,\delta$$ (checked by the gtests `WeightedGap3`/`WeightedGap4` on the explicit `WEIGHTED_SLIP`, which uses the same expressions). The generator also builds `NormalwGap` $$= +\mathbf{n} \cdot (\mathbf{D}\mathbf{w}^1 - \mathbf{M}\mathbf{w}^2)$$ and `TangentwSlipObjective` / `TangentwSlipNonObjective`, which are **minus** the variations of the corresponding quantities in the direction of the test functions ($$X_w = -\delta X$$, the virtual work of a traction $$\mathbf{t}$$ being written $$\mathbf{t} \cdot X_w$$); the penalty frictional functional uses them, the ALM one expresses the virtual work through $$\bar{\boldsymbol{\lambda}}_j \cdot (\mathbf{D}\delta\mathbf{u}^1 - \mathbf{M}\delta\mathbf{u}^2)_j$$ instead. The **five branches** generated per node are then
...
The generated code (the first specialisation of `custom_conditions/ALM_frictional_mortar_contact_condition.cpp`) dispatches per node with nested `if`s rather than through the `rActiveInactive` integer:
...
`mPreviousMortarOperators` (type `MortarOperator<TNumNodes,TNumNodesMaster>`) holds $$\mathbf{D}^t$$, $$\mathbf{M}^t$$; `mPreviousMortarOperatorsInitialized` is reset in `FinalizeSolutionStep` and the operators are recomputed on demand by `ComputePreviousMortarOperators` (template `ALM_frictional_mortar_contact_condition_template.cpp:139`, delegating to `MortarExplicitContributionUtilities::ComputePreviousMortarOperators`, which integrates $$\mathbf{D}$$, $$\mathbf{M}$$ on the coordinates of the previous step). The `StepSlip = TNormalVariation ? 0 : 1` constant of the header is the buffer position historically associated with the slip; the currently generated code does not use it, the previous configuration $$\mathbf{x}^{1,t}$$, $$\mathbf{x}^{2,t}$$ being taken from `DerivativeDataFrictional`: `u1old`/`u2old` are the offsets from `X1`/`X2` (which, after the first step, are already the previous-step coordinates of `DerivativeData`) to the previous position, i.e. `x1old = X1 + u1old` is always the position of **buffer position 1** and `x1 - x1old` the displacement increment of the step, exactly the increment used by the explicit `WEIGHTED_SLIP`. (Until 2026 `u1old` was the previous *increment* `DISPLACEMENT(1) - DISPLACEMENT(2)`, which made the residual's non-objective slip use `Δu_cur − Δu_prev` and required a buffer size of 3; the default buffer size of 2 is now enough.) Both members are serialised, so restarts keep the slip history.
...
The corresponding solver-side settings are `mortar_type` (`ALMContactFrictional`, `ALMContactFrictionalPureSlip`, `PenaltyContactFrictional`, `PenaltyContactFrictionalPureSlip`), the automatic insertion of `MixedULMLinearSolver` for `ALMContactFrictional*`, and the variables/DoFs added by `AuxiliaryAddVariables`/`AuxiliaryAddDofs` (`VECTOR_LAGRANGE_MULTIPLIER` with reaction `WEIGHTED_VECTOR_RESIDUAL`, `WEIGHTED_GAP`, `WEIGHTED_SLIP`). A minimal frictional set-up (from `tests/ALM_frictional_contact_test_2D/hyper_simple_patch_test_parameters.json`):
...
- Frictional runs need, for the ALM, the `MixedULMLinearSolver` or a direct solver able to handle the saddle-point structure (the default buffer size of 2 is enough).

...
- Frictional problems need `FRICTION_COEFFICIENT` per interface (`friction_coefficients`); the default `buffer_size` of 2 is enough (the previous configuration is read from buffer position 1).
...
| `tests/ALM_frictional_contact_test_3D/friction_block_test` (`TALMBlockTestFrictionalContact`, validation) | The slip/stick history of this 3D case depends on the number of OpenMP threads (runs are reproducible for a fixed thread count, but 1-thread and 4-thread results differ far beyond round-off). | The reference was regenerated single-threaded (`OMP_NUM_THREADS=1`); the test fails with several threads, as it did before. `ALMSelfContactContactTest` / `ComponentsALMSelfContactContactTest` (search: `mPointListDestination not initialized`) and the MPC `PlateTest` also fail in the validation suite independently of the contact formulation. |
| `custom_conditions/penalty_frictional_mortar_contact_condition.h` (`ComputeTangentMatrixSlip`) | Calls `MortarUtilities::ComputeTangentMatrix<TDim, TNumNodes>` with the template arguments in the wrong order (the utility is `<TNumNodes, TDim>`). | Only the 3D quadrilateral penalty-frictional conditions (`TNumNodes = 4`) are affected: the loop over the tangent components runs out of the `array_1d<double, 3>`. Not fixed here because the header is hand-written; the 2D and 3D triangle cases are correct. |
| `README.md` of the application | Advertised test counts are outdated (the runner registers 123 active tests, 91 gtests). | Documentation only. |

## Resolved defects (kept for reference)

| Where | Issue | Resolution |
|---|---|---|
| `kratos/includes/mortar_classes.h` (`DerivativeDataFrictional::u1old/u2old`) and the non-objective slip of the frictional conditions | Since the 2017 change to the updated-Lagrangian split (`X1` is the previous position after the first step, `u1` the increment), `u1old` was the *previous increment* `DISPLACEMENT(1) - DISPLACEMENT(2)`, so the generated residual's non-objective slip `-[D(x1 - x1old) - M(x2 - x2old)]` used `Δu_cur - Δu_prev` instead of the step increment `Δu_cur` of the explicit `WEIGHTED_SLIP`. It only affected the non-objective **stick** branches from the second step on. | `u1old`/`u2old` are now the offset from `X1`/`X2` to the previous position (`GetCoordinates(…, false, 1) - GetCoordinates(…, false, step)`), so `x1old = X1 + u1old` is always the previous position. `DISPLACEMENT(2)` is no longer read, hence the `buffer_size >= 3` requirement of the frictional solvers (whose enforcing check in `auxiliary_methods_solvers.py` was inverted and never bumped the buffer) is gone. The multi-step frictional references were regenerated. |
| Legacy branch `contact/wrong-sign-non-objective-friiction` | Flipped the sign of `gap_time_derivative_non_objective_w` in the frictional generator. | Not applied: that symbol only feeds `TangentwSlipNonObjective`, which no functional uses, so the regenerated code was numerically identical; and under the convention of the generators (`Xw = -δX`, as `NormalwGap`) the original sign is the consistent one. |
| `automatic_differentiation/*` | The generators needed sympy 1.2 (symbol-to-function conversion removed in 1.3) and could not reproduce the hand-made sharing of the RHS between the two normal-variation specialisations. | Rewritten on top of `sympy.Function` with a shared `mortar_condition_generator.py`; the RHS sharing is generated. The historical frictionless-components generator built the multiplier projections before injecting the DoF dependency of the normal, so its normal-variation tangent lacked those terms; the regenerated tangent is consistent (residuals unchanged). |
| Frictional generated preambles | `MOperatorold` and `DeltaMOperator` were declared `<TNumNodes, TNumNodes>` instead of `<TNumNodes, TNumNodesMaster>`, binding a converting temporary for the mixed 3N/4N specialisations. | Fixed in the regenerated files. |

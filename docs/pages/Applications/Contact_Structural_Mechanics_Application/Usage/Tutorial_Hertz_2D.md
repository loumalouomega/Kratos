...
|---|---|---|
| **Frictional contact** (Coulomb) | `tests/ALM_frictional_contact_test_2D/hyper_simple_patch_test_parameters.json` (8-node patch test, 1 step) | `contact_settings.mortar_type: "ALMContactFrictional"`; in the process `"contact_type": "Frictional"` and `"friction_coefficients": {"0": 0.01}`. The solver uses the vector multiplier `VECTOR_LAGRANGE_MULTIPLIER`; the convergence table gains stick/slip columns. Optional keys: `tangent_factor`, `slip_threshold`, `contact_type: "FrictionalPureSlip"`. |
